...
// 传入 fd，需要 hook 的函数名，ioevent。模板函数
...
retry:
    ...
    if(n == -1 && errno == EAGAIN) {
        ...
        if(rt > 0){
            ...
            // 添加成功则 yield ，唤醒之后如果还有定时就取消掉，再强制触发
