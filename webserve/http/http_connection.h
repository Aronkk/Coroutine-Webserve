...
#include "webserve/streams/socket_stream.h"
#include "http.h"
#include "webserve/uri.h"
...
public:
    ...
    /**
     * @brief 发送HTTP的GET请求，这样就不用指定 method
     * @param[in] url 请求的url
...
private:
    std::string m_host;
    std::string m_vhost;
    uint32_t m_port;            // 连接池的端口
    uint32_t m_maxSize;         // 连接池的上限，最多存多少条连接
    uint32_t m_maxAliveTime;    // 连接池的最大连接时间（如30s）
    uint32_t m_maxRequest;      // 每个连接的最大请求上限（如一个连接最多请求2000次）
    bool m_isHttps;

    MutexType m_mutex;
    std::list<HttpConnection*> m_conns;     // 连接池
    std::atomic<int32_t> m_total = {0};     // 连接池的容量
};
